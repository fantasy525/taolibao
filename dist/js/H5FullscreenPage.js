(function(){
         ...
         var obj = {
             ...
             '3' : {
                 'upDrag' : function(percentage, item){
                     var translateY = 1 - 0.5*percentage;//位置系数，可以微调
         ...
         function touchMove (event) {
            ...
            if (percentage > 0) {
                // //向上拖动
                console.log('向上',percentage,dragStart,event.clientY)
         ...
         function initDom(opt){
            ...
            if (opt.useArrow) {
                $('.item').slice(0,$('.item').length-1).append('<span class="arrow"></span><br><span' +
                    ' class="arrow"></span>');
                $('.arrow').on('click',function () {
                    $(this).parents('.item').css('-webkit-transform', 'translate3d(0,-100%,0)');
                    $(this).parents('.item').next().css('-webkit-transform', 'translate3d(0,0%,0)'); //下一个item上移动
                })
            }
            if (opt.useMusic) {
